...
<!DOCTYPE html><html lang="en"><head>
  ...
  <title>Record Payment — Tie The Tech</title>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Outfit',sans-serif;min-height:100vh;background:linear-gradient(135deg,#1a0533,#2d1b69,#0d2137);padding:32px 20px;color:#fff}
    ...
    h2{font-size:24px;font-weight:700;margin-bottom:28px}.form-row{display:grid;grid-template-columns:1fr 1fr;gap:16px}.form-group{margin-bottom:20px}
...
<nav class="nav"><span class="brand">💍 Tie The Tech</span><a href="/payments">← Back to Payments</a></nav>
<div class="card">
  <h2>💳 Record Payment</h2>
  <form method="post" action="/payments/new">
    <div class="form-row">
      <div class="form-group"><label>Booking ID</label><input type="text" name="bookingId" id="bookingId" placeholder="e.g. BKG-XXXXX" required></div>
      <div class="form-group"><label>Amount (LKR)</label><input type="number" name="amount" id="amount" min="0" step="0.01" placeholder="0.00" required></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label>Payment Method</label>
        <select name="paymentMethod" id="paymentMethod" required><option value="">Select...</option><option>Cash</option><option>Bank Transfer</option><option>Card</option><option>Cheque</option></select>
      </div>
      <div class="form-group"><label>Payment Date</label><input type="date" name="paymentDate" id="paymentDate" required></div>
    </div>
    <div class="form-group"><label>Status</label>
      <select name="status" id="status"><option value="PAID">Paid</option><option value="PENDING">Pending</option></select>
    </div>
